...
case class AlgorithmParams(dimensions: Int, yearWeight: Double,
                           ratingsWeight: Double, normalizeProjection:
...
class Algorithm(val ap: AlgorithmParams)
  ...
  def train(sc: SparkContext, data: PreparedData): Model = {
    ...
    val categorical = Seq(encode(data.items.map(_._2.directors)),
      encode(data.items.map(_._2.writers)),
      encode(data.items.map(_._2.genres)),
      encode(data.items.map(_._2.actors)),
      encode(data.items.map(_._2.language)),
      encode(data.items.map(_._2.budget.toString())))
    ...
    val numericRow = data.items.map(x => Vectors.dense(x._2.year, x._2
      .ratings))
    val weights = Array(ap.yearWeight, ap.ratingsWeight)
